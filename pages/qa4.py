...
# Load user data
data_path = f"data/{username}/qa4.csv"
...
assert (
    len(id_unfinished)
    == user_metadata["qa4"]["total_items"] - user_metadata["qa4"]["completed_items"]
)
...
def save_data():
    ...
    # Update the metadata
    user_metadata["qa4"]["completed_items"] += 1
...
if next_button:
    ...
    st.markdown(f'<p class="serif-font">{report_text}</p>', unsafe_allow_html=True)
